...
class PlayerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //시간당 스케일을 생각해라 1초에 10개로 분할, 즉 0.1초
...
extension PlayerViewController {
    func updateTrackInfo() {
        // TODO: 트랙 정보 업데이트
        guard let track = self.simplePlayer.currentItem?.convertToTrack() else {
            print("can't get")
            return
        }
        
        thumbnailImageView.image = track.artwork
        titleLabel.text = track.albumName
    ...
    func updatePlayButton() {
        ...
        if simplePlayer.isPlaying {
            let configureation = UIImage.SymbolConfiguration(pointSize: 40)
            let image = UIImage(systemName: "pause.fill", withConfiguration: configureation)
            playControlButton.setImage(image, for: .normal)
        } else {
            let configureation = UIImage.SymbolConfiguration(pointSize: 40)
            let image = UIImage(systemName: "play.fill", withConfiguration: configureation)
            playControlButton.setImage(image, for: .normal)
        }
    }
}
